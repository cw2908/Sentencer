...
describe("Sentencer:", function () {
  ...
  describe("Default", function () {
    ...
    describe("# Actions", function () {
      it("should include `noun`", function () {
        assert(Sentencer.actions.noun);
      });
      it("should include `verb`", function () {
        assert(Sentencer.actions.verb);
      });
      it("should include `adverb`", function () {
        assert(Sentencer.actions.adverb);
      });
      it("should include `a_noun`", function () {
        assert(Sentencer.actions.a_noun);
      });
      it("should include `nouns`", function () {
        assert(Sentencer.actions.nouns);
      });
      it("should include `adjective`", function () {
        assert(Sentencer.actions.adjective);
      });
      it("should include `an_adjective`", function () {
        assert(Sentencer.actions.an_adjective);
      });
    });
  });
  ...
  describe("API", function () {
    ...
    it("should accept another action merge later", function () {
      ...
      assert.equal(
        Sentencer.actions.firstNewAction(),
        "hello",
        "first action still exists"
      );
      assert.equal(
        Sentencer.actions.secondNewAction(),
        "hello again",
        "second action exists as well"
      );
    });
  ...
  describe("Templating", function () {
    describe("# Default Actions", function () {
      it("{{ noun }}", function () {
        assert(Sentencer.make("{{ noun }}"));
      });
      it("{{ a_noun }}", function () {
        assert(Sentencer.make("{{ a_noun }}"));
      });
      it("{{ nouns }}", function () {
        assert(Sentencer.make("{{ nouns }}"));
      });
      it("{{ adjective }}", function () {
        assert(Sentencer.make("{{ adjective }}"));
      });
      it("{{ an_adjective }}", function () {
        assert(Sentencer.make("{{ an_adjective }}"));
      });
      it("{{ verb }}", function () {
        assert(Sentencer.make("{{ verb }}"));
      });
      it("{{ adverb }}", function () {
        assert(Sentencer.make("{{ adverb }}"));
      });
    });
    ...
    describe("# Custom Actions", function () {
      ...
      it("should return {{ action }} if it does not exist", function () {
        assert.equal(
          Sentencer.make("{{ nonexistant thing }}"),
          "{{ nonexistant thing }}"
        );
      });
    });
  ...
  describe("Test Print", function () {
    it("should have logged a sentence", function () {
      console.log(
        Sentencer.make(
          "      Here is {{ an_adjective }} sentence generated by Sentencer's {{ nouns }}."
        )
      );
    });
  });
